...
type WorldOpsView = "radar" | "balance" | "tuning" | "runs";
...
const VIEW_OPTIONS: Array<{ label: string; value: WorldOpsView }> = [
...
function postureLabel(posture: WorldOpsPayload["health"]["risk_posture"]) {
  if (posture === "hot") return "Alta fricción";
  if (posture === "watch") return "En observación";
  return "Normal";
}
...
export default function WorldOpsPanel({ initialOps }: Props) {
  ...
  const [activeView, setActiveView] = useState<WorldOpsView>("radar");
    ...
    >
      ...
      <div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 font-display text-3xl leading-none md:text-4xl">Riesgo, economía y runs</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/[0.58]">
            Lectura operacional de las partidas emitidas por servidor, cobertura de auditoría y recompensas pagadas.
          </p>
        </div>
      ...
      {!ops ? (
        <div className="relative mt-5 rounded-xl border border-white/10 bg-white/[0.03] p-5 text-sm text-white/50">
          Sin datos de World Ops todavía.
        </div>
      ) : (
        <div className="relative mt-5 space-y-4">
          <div className="grid grid-cols-4 gap-1.5 rounded-xl border border-white/10 bg-black/20 p-1.5" role="tablist" aria-label="Vistas de World Ops">
            {VIEW_OPTIONS.map((view) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-10 rounded-lg px-2 text-xs font-black transition-all ${
                    active
          ...
          {activeView === "radar" && (
            <RadarView maxTrend={maxTrend} ops={ops} />
          )}

          {activeView === "balance" && (
            <BalanceOverview ops={ops} />
          )}
...
function RadarView({ maxTrend, ops }: { maxTrend: number; ops: WorldOpsPayload }) {
  return (
...
function RunsView({ ops }: { ops: WorldOpsPayload }) {
  return (
...
function BalanceOverview({ ops }: { ops: WorldOpsPayload }) {
    ...
    <div className="rounded-2xl border border-amber-300/15 bg-[#151107] p-4 shadow-[0_24px_80px_-56px_rgba(245,158,11,.7)] md:p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h3 className="mt-2 font-display text-3xl leading-none">Economía, dificultad y retención</h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/[0.58]">
            Diagnóstico read-only: nodos fríos, fricción alta, payout relativo y recomendaciones antes de lanzar eventos.
          </p>
        </div>
    ...
    <div className="rounded-2xl border border-amber-300/15 bg-[#151107] p-4 shadow-[0_24px_80px_-56px_rgba(245,158,11,.7)] md:p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h3 className="mt-2 font-display text-3xl leading-none">Ajustes auditados</h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-white/[0.58]">
            Cambios controlados con preview obligatorio, motivo de balance e historial de auditoría.
          </p>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:min-w-[520px]">
          <BalancePill
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.035] p-4">
          ...
          <div className="mt-4 rounded-xl border border-white/[0.08] bg-black/[0.18] p-3">
            ...
            <div className="mt-3 space-y-2">
              {history.length === 0 ? (
                <div className="text-sm font-bold text-white/[0.42]">Todavía no hay ajustes aplicados.</div>
              ) : (
        ...
        <div className="rounded-xl border border-amber-300/15 bg-black/[0.22] p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span className="text-[11px] font-mono text-white/[0.35]">
              Preview obligatorio · motivo mínimo 8 chars
            </span>
          </div>
          ...
          <div className="mt-4 rounded-xl border border-white/[0.08] bg-white/[0.035] p-3">
            {preview ? (
              ...
              <div className="flex items-center gap-2 text-sm font-bold text-white/[0.45]">
                ...
                Ajusta valores y genera preview para ver impacto económico antes de aplicar.
              </div>
            )}
          </div>
        </div>
...
function MetricCard({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.04] p-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-cyan-200/25 hover:bg-white/[0.065] sm:p-4">
      <div className="flex items-center justify-between gap-3">
        <div className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-black/20 text-cyan-100 sm:h-9 sm:w-9">
          {icon}
        </div>
        <div className="text-right text-[9px] font-mono uppercase tracking-[0.1em] text-white/[0.36] sm:text-[10px] sm:tracking-[0.14em]">{label}</div>
      </div>
      <div className="mt-3 font-display text-2xl leading-none sm:text-3xl">{value}</div>
