...
export default function StoreClient({
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10">
      ...
      <Section
        title="Gold Coins"
        subtitle="Diversión sin riesgo · Solo para jugar"
        emoji="🪙"
        color="#FFD93D"
      >
      ...
      <Section
        title="Sweeps Coins"
        subtitle="Para premios reales · Canjeables por USD"
        emoji="💎"
        color="#FF3D7F"
      >
        <div className="grid sm:grid-cols-3 gap-3">
          {sweepsPackages.map((p, i) => (
            <PackageCard
              key={p.id}
              pkg={p}
              loading={loading === p.id}
              onBuy={() => handleBuy(p.id)}
              amountLabel={`💎 ${Number(p.sweeps_coins).toFixed(0)} SC`}
              isPopular={i === 1}
              accent="#FF3D7F"
            />
          ))}
        ...
        <div className="mt-3 text-xs text-[var(--color-fg-muted)] text-center">
          ⚠️ Sweeps Coins requieren verificación KYC para canjear · Sólo disponible en estados habilitados
        </div>
      </Section>
      ...
      <div className="mt-12 text-center text-xs text-[var(--color-fg-muted)] leading-relaxed max-w-2xl mx-auto">
        🔒 Pagos seguros con Stripe · 🇺🇸 Disponible en 45 estados · 🎮 Juego responsable · 21+
        <br />
...
function PackageCard({ pkg, loading, onBuy, amountLabel, isPopular, accent, bonus }: {
  pkg: CoinPackage;
  loading: boolean;
  onBuy: () => void;
  amountLabel: string;
  ...
  return (
    ...
    >
      ...
      <div className="font-display text-xl md:text-2xl mb-3 shimmer-gold">
        {amountLabel}
      </div>
      <button
        onClick={onBuy}
