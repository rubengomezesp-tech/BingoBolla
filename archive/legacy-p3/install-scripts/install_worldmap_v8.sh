...
export default function WorldMap({
  ...
  function playNode(n: Node) {
    if (n.node_type === "bingo") {
      // Partida real: si el nodo tiene sala asignada, entra ahi.
      // Si no, primera sala disponible (fallback, como HomeScreen).
      if (n.target_ref) {
        router.push(`/room/${n.target_ref}`);
      } else {
        const firstBingo = nodes.find((x) => x.node_type === "bingo" && x.target_ref);
        router.push(firstBingo?.target_ref ? `/room/${firstBingo.target_ref}` : "/lobby#salas");
      }
    } else {
      setSelected(null);
    }
    ...
    <div className="wm-root">
      ...
      <div className="wm-hud">
        ...
        <div className="wm-resrow">
          ...
          <div className="wm-res">
            <div className="wm-ic wm-ic-gem">💎</div>
            <div className="wm-val">{fmt(diamonds)}</div>
            <div className="wm-add">+</div>
          </div>
        </div>
          ...
          nodes.map((n) => {
            ...
            return (
              ...
              >
                {state === "locked" ? (
                  assets["node-locked"] ? (
                    <img className="wm-lockimg" src={assets["node-locked"]} alt="Bloqueado" />
                  ) : (
...
.wm-lockimg{width:118%;height:118%;object-fit:contain;
  filter:drop-shadow(0 3px 7px rgba(0,0,0,.6));
  animation:wmLockGlow 2.8s ease-in-out infinite;}
@keyframes wmLockGlow{0%,100%{filter:drop-shadow(0 3px 7px rgba(0,0,0,.6)) brightness(1)}
  50%{filter:drop-shadow(0 3px 10px rgba(255,80,160,.5)) brightness(1.12)}}
