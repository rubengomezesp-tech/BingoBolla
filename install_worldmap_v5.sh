#!/bin/bash
# BINGOBOLLA · WorldMap v5: nodo activo oculto (solo mascota) + mascota x2
cd ~/bingobolla || { echo "No existe ~/bingobolla"; exit 1; }
echo "1/3 · Backup..."
BACKUP="src/components/WorldMap.tsx.bak.v5-$(date +%Y%m%d-%H%M%S)"
cp src/components/WorldMap.tsx "$BACKUP"
echo "    Backup: $BACKUP"
echo "2/3 · Escribiendo v5..."
...
if [ $RESULT -eq 0 ]; then
  ...
  echo "Subir:  git add -A && git commit -m \"WorldMap v5: mascota grande sobre nodo\" && git push origin main && vercel --prod"
else
